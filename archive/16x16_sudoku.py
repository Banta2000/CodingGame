...
class SUDOKU:
    FULL_SET = set(["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", "P"])

    def __init__(self, init_string_list: list[str]):
        self.board = [[x for x in line] for line in init_string_list]
        self.lookup = [[None for _ in range(16)] for _ in range(16)]
        self._update_lookup()
    ...
    def _calculate_available_letters_of_a_pos(self, row: int, col: int) -> set[str]:
        ...
        col_letters = set(self.board[r][col] for r in range(16))
        min_row, min_col = (row // 4) * 4, (col // 4) * 4
        quadrant_letters = {self.board[r][c] for r in range(min_row, min_row + 4) for c in range(min_col, min_col + 4)}
    ...
    def count_missing_fields(self):
        return sum([line.count(".") for line in self.board])
    ...
    def insert_single_letters(self):
        ...
        while made_a_change:
            ...
            filled_out_start = sum([line.count(".") for line in self.board])
            ...
            # Inserts letters that appear only once in a given row
            for row in range(16):
                selection = [(row, col) for col in range(16) if self.board[row][col] == "."]
                _insert_single_letters_in_selection(selection)

            # Inserts letters that appear only once in a given col
            for col in range(16):
                selection = [(row, col) for row in range(16) if self.board[row][col] == "."]
                _insert_single_letters_in_selection(selection)
            ...
            for row in [0, 4, 8, 12]:
                for col in [0, 4, 8, 12]:
                    selection = [
                        (r, c) for r in range(row, row + 4) for c in range(col, col + 4) if self.board[r][c] == "."
                    ]
                    _insert_single_letters_in_selection(selection)

            # Count number of filled out fields
            filled_out_end = sum([line.count(".") for line in self.board])
...
game = load_input("test2")

game.insert_single_letters()
