...
def get_start_parameters():
    ...
    defibs = [parse_defib(line) for line in data]

    # defibs = []
    # for defib in data:
    #     defib = defib.split(";")
    #     name = defib[1]
    #     lon = float(defib[4].replace(",", "."))
    #     lat = float(defib[5].replace(",", "."))
    #     defibs.append([name, lon, lat])

    return defibs, LON, LAT


def dist(lona, lata, lonb, latb):
    x = (lonb - lona) * math.cos(0.5 * (lata + latb))
    y = latb - lata
...
def parse_defib(line: str) -> Defibrillator:
    parts = line.split(";")
    return Defibrillator(name=parts[1], lon=float(parts[4].replace(",", ".")), lat=float(parts[5].replace(",", ".")))


DEFIBS, LON, LAT = get_start_parameters()

closest = min(DEFIBS, key=lambda d: dist(LON, LAT, d.lon, d.lat))
print(closest.name)
